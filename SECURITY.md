...
- Suite baseline comparison limits each imported report to 8 MiB, rejects unexpected or malformed report fields, requires unique stable case IDs, recomputes case/finding summaries and effective strict-policy status, and compares normalized values without evaluating imported strings. Exact report hashes bind bytes but do not authenticate their producer or re-verify the original blueprint artifacts.
- Suite-diff JUnit and Markdown projections normalize control characters and context-escape imported strings before serialization. The JUnit file is a compact comparison view, not runtime test evidence; the Markdown file cannot directly turn imported metacharacters into links or remote images.
- Declared suite-change plans are limited to 1 MiB and require bounded exact case/dimension declarations, a baseline SHA-256, a real date-only expiry, an explicit review date, and a credential-free HTTPS or URN reference. The evaluator verifies that the parsed plan matches the exact hashed bytes, but it does not authenticate the asserted owner, reference, date, or authority.
...
- Treat generated suite-diff JSON, JUnit, and Markdown as untrusted artifacts in downstream CI systems. Preserve the CLI exit code as the gate, keep richer JSON evidence when needed, and do not reinterpret a JUnit testcase as proof that an agent or contract executed.
- Treat a passing declared-change intent gate as exact drift agreement, not permission to override a failing comparison gate. Enforce who may adopt that policy through authenticated repository rules, review requirements, and short-lived change plans; do not use wildcard or open-ended exceptions.
